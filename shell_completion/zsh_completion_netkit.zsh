#compdef netkit network-toolkit nt
...
_netkit_complete() {
  ...
  case $state in
    cmds)
  values=(${(f)"$(netkit __complete --for commands 2>/dev/null)"})
      _describe -t commands 'netkit commands' values && return
      ;;
    args)
      case $words[2] in
        info)
          values=(${(f)"$(netkit __complete --for devices --config \"$cfg\" 2>/dev/null)"})
        ...
        run)
          if (( CURRENT == 3 )); then
            ...
            groups=(${(f)"$(netkit __complete --for groups --config \"$cfg\" 2>/dev/null)"})
            devices=(${(f)"$(netkit __complete --for devices --config \"$cfg\" 2>/dev/null)"})
            annotated=()
            local g d
            ...
            values=(${(f)"$(netkit __complete --for sequences --device \"$target\" --config \"$cfg\" 2>/dev/null)"})
        ...
        upload|download|config-backup|backup|firmware-upgrade|firmware-downgrade|bios-upgrade|diff)
          if (( CURRENT == 3 )); then
            values=(${(f)"$(netkit __complete --for devices --config \"$cfg\" 2>/dev/null)"} ${(f)"$(netkit __complete --for groups --config \"$cfg\" 2>/dev/null)"})
...
compdef _netkit_complete netkit network-toolkit nt
