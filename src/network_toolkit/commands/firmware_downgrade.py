...
to upgrade from the device's perspective (RouterOS applies what is present on reboot).
"""
...
def register(app: typer.Typer) -> None:
    ...
    def firmware_downgrade(  # pyright: ignore[reportUnusedFunction]
        ...
        *,
        ...
        This calls '/system package downgrade' after uploading the .npk, then
        reboots the device with automatic confirmation handling.
        """
            ...
            def process_device(dev: str) -> bool:
                try:
                    with device_session(dev, config) as session:
                        ...
                        ok = session.downgrade_firmware_and_reboot(
                        ...
                        if ok:
                            ...
                            return True
                        console.print(
                            f"[red]✗ Firmware downgrade failed to start on {dev}[/red]"
                        )
                        return False
