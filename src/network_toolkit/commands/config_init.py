...
def register(app: typer.Typer) -> None:
  ...
  def config_init(
    ...
    if target_dir is not None:
      ...
      if yes:
        ...
        if typer.confirm("Use default location?", default=True):
          ...
          entered = typer.prompt("Enter a directory path", default=str(default_path))
    ...
    if not force:
      ...
      if existing:
        if not interactive:
          ...
          console.print("[yellow]Existing configuration detected in the default location.[/yellow]")
    ...
    def _install_sequences_from_repo(url: str, ref: str, dest: Path) -> None:
      ...
      try:
        subprocess.run(["git", "clone", "--depth", "1", "--branch", ref, url, str(tmp_root)], check=True, capture_output=True)
        ...
        if src.exists():
          ...
          _print_success(f"Installed additional sequences from {url}@{ref} into: {dest}")
        else:
          _print_warn("No sequences found in repo under config/sequences; skipping.")
    ...
    def _install_shell_for(selected: str) -> tuple[Path, Path | None]:
      ...
      _print_warn("Only bash and zsh shells are supported for completion installation.")
      return (Path(""), None)

    def _activate_shell(selected: str, installed: Path, rc_file: Path | None) -> None:
      if rc_file is None:
        return
      begin = "# >>> NW COMPLETION >>>"
      end = "# <<< NW COMPLETION <<<"
    ...
    if install_sequences is not None:
      ...
      do_install_sequences = typer.confirm(
        "Install additional predefined vendor sequences from GitHub?", default=True
      )
    ...
    if do_install_compl:
      detected = _detect_shell() if interactive else (shell if shell in {"bash", "zsh"} else None)
      if interactive:
        default_shell = detected or "bash"
        answer = typer.prompt("Choose shell for completions (bash|zsh)", default=default_shell)
      ...
      if activate_completions is not None:
        ...
        do_activate_compl = typer.confirm("Activate completions by updating your shell rc file?", default=True)
    ...
    if dry_run:
      ...
      _print_action("[dry-run] Would create config.yml/devices/groups and basic sequences")
    else:
    ...
    if do_install_sequences:
      ...
      if dry_run:
        _print_action(f"[dry-run] Would download sequences from {chosen_seq_url}@{git_ref} into: {sequences_dir}")
      else:
    ...
    if do_install_compl and chosen_shell:
      if dry_run:
        _print_action(f"[dry-run] Would install {chosen_shell} completion and activate={do_activate_compl}")
      else:
        installed_path, rc_file = _install_shell_for(chosen_shell)
        console.print(f"Installed {chosen_shell} completion script to: {installed_path}")
        if rc_file:
          console.print(f"Shell rc file detected: {rc_file}")
        if do_activate_compl:
          _activate_shell(chosen_shell, installed_path, rc_file)
          console.print("Activation snippet appended. Open a new shell to use completions.")
        else:
          console.print("Activation skipped. You can manually source or add the snippet later.")
